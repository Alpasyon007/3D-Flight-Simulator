...
void Matrix4x4::setVector3f(Vector3f v) {
	this->val[3][0] = v.x;
	this->val[3][1] = v.y;
	this->val[3][2] = v.z;
}

Vector3f Matrix4x4::getVector3f() {
	float x, y ,z;
...
Vector3f Matrix4x4::getRotation() {
	float yaw, pitch, roll;

	// yaw = atan2f(this->val[2][1], this->val[1][1]) * 180/M_PI;
	// pitch = acosf(this->val[2][2]) * 180/M_PI;
	// roll = atan2f(this->val[0][2], this->val[1][2]) * 180/M_PI;

	// if(this->val[0][0] == 1.0f || this->val[0][0] == -1.0f) {
	// 	yaw = atan2f(this->val[2][0], this->val[3][2]) * 180/M_PI;
	// 	pitch = 0;
	// 	roll = 0;
	// } else  {
		yaw = atan2(this->val[0][2], this->val[0][0]) * 180/M_PI;
		pitch = atan2(this->val[2][1], this->val[1][1]) * 180/M_PI;
		roll = atan2(this->val[0][1], this->val[1][1]) * 180/M_PI;
	// }

	Vector3f vector(yaw,pitch,roll);
	return vector;
}


Vector3f Matrix4x4::getRight() {
	float x, y ,z;
